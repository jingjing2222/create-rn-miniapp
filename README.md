...
- `packages/trpc`: optional tRPC shared workspace를 만들고, client와 server가 같은 `AppRouter` 타입을 보게 해줘요.
...
- tRPC를 같이 고르면 `packages/trpc`를 canonical source로 만들고, `frontend`/`backoffice`는 상대 경로 대신 shared type만 가져오게 맞춰줘요.
...
- `--trpc`: `supabase`나 `cloudflare` 위에 optional tRPC overlay를 같이 만들어요.
...
`supabase`나 `cloudflare`를 고르면 tRPC도 같이 이어줄지 물어봐요. tRPC를 고르면 `packages/trpc`가 router와 `AppRouter` 타입의 source of truth가 되고, `frontend`/`backoffice`는 `src/lib/trpc.ts`를 통해 그 타입만 가져와요.
...
`packages/trpc`를 같이 고르면:
- `packages/trpc/`
- `server/supabase/functions/api/deno.json`
...
- tRPC를 같이 고르면 `packages/trpc`가 canonical router가 되고, `server/supabase/functions/api/deno.json`의 `imports`가 `@workspace/trpc`를 `packages/trpc/src/index.ts`로 직접 연결해줘요.
...
- `.env.local`
- `src/env.d.ts`
- `src/lib/api.ts` # plain mode
- `src/lib/trpc.ts` # tRPC mode
- `granite.config.ts` env plugin 및 monorepo `watchFolders` patch
...
- tRPC를 같이 고르면 `packages/trpc`가 canonical router가 되고, Worker runtime은 `@workspace/trpc`를 직접 import해서 같은 router를 바로 써요. 이 경우 client 기본 진입점도 `src/lib/trpc.ts`예요.
...
- `server/package.json`의 `deploy`는 `server/functions` 의존성을 설치한 뒤 `server/.env.local`의 auth 값을 읽어 `firebase-tools deploy --only functions`를 실행해요.
