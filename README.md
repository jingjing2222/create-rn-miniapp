...
worktree 레이아웃을 골랐다면 실제 repo root는 `my-miniapp/main`이 돼요. control root에는 로컬 안내용 `AGENTS.md`, `README.md`, `.git`, `.bare/`만 남고, 이후 worktree 관리는 `git worktree` 명령으로 진행하면 돼요.
...
<appName>/
  ...
  main/
    ...
    nx.json
    biome.json
  feat-a/       # `git worktree add -b feat-a ./feat-a main` (control root에서)
```

이 구조에서는 control root에서 `git worktree list`, `git worktree add -b <branch> ./<branch> main`, `git worktree remove <path>` 같은 명령으로 worktree를 관리하고, 실제 코드 수정과 `pnpm verify`는 `main/`이나 추가 worktree 안에서 진행하면 돼요. `main/`에서 `git pull` 하면 merged된 worktree가 자동으로 정리돼요 (post-merge hook). 변경사항이 남아있는 worktree는 건너뛰어요.
