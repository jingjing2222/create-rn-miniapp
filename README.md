...
- 기존 프로젝트를 고르면 해당 프로젝트로 `server`만 link 해요. 기존 프로젝트는 원격 migration history와 기존 Edge Function 상태가 이미 있을 수 있어서 `db push`와 기본 Edge Function deploy는 자동으로 건너뛰고, 필요하면 `server`에서 `db:apply`, `functions:deploy`를 직접 실행하면 돼요.
