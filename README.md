...
- `packages/trpc/`
- `server/scripts/trpc-sync.mjs`
- `server/supabase/functions/_shared/trpc/`
...
- tRPC를 같이 고르면 `packages/trpc`가 canonical router가 되고, `server/scripts/trpc-sync.mjs`가 Supabase Edge Functions runtime mirror를 맞춰줘요.
...
- tRPC를 같이 고르면 `packages/trpc`가 canonical router가 되고, Worker runtime은 `@workspace/trpc`를 직접 import해서 같은 router를 바로 써요.
