...
<appName>/
  ...
  main/
    ...
    nx.json
    biome.json
  feat-a/       # `git worktree add -b feat-a ../feat-a main` 예시
```

이 구조에서는 control root에서 `git worktree list`, `git worktree add -b <branch> ../<branch> main`, `git worktree remove <path>` 같은 명령으로 worktree를 관리하고, 실제 코드 수정과 `pnpm verify`는 `main/`이나 추가 worktree 안에서 진행하면 돼요.
