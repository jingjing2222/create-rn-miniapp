...
- `packages/contracts`: optional Cloudflare tRPC boundary schema workspace를 만들고, client-server 경계 타입을 `z.infer` 기준으로 공유하게 해줘요.
- `packages/app-router`: optional Cloudflare tRPC router workspace를 만들고, client와 server가 같은 `AppRouter` 타입을 보게 해줘요.
...
- `server/firestore.rules`, `server/firestore.indexes.json`, `server/firestore.seed.json`과 `server/scripts/firebase-firestore-seed.mjs`도 같이 만들어서 Firestore 설정과 seed 흐름을 바로 이어갈 수 있게 해줘요.
- `server/package.json`의 `deploy`는 `server/functions` 의존성을 설치한 뒤 `server/.env.local`의 auth 값을 읽어 `firebase-tools deploy --only functions,firestore`를 실행해요.
- `server/package.json`의 `firestore:seed`는 `server/firestore.seed.json`을 읽어 Firestore에 기본 문서를 적재해요.
