...
- a56e751: 추천 agent skills 설치 시 `tds-ui` 설치본에 `metadata.json`이 없어도 CLI에 내장된 metadata 계약으로 llms mirror를 계속 동기화하도록 고쳤습니다.

## 0.1.9

### Patch Changes

- 305b27d: 추천 agent skills 설치 뒤 `tds-ui` 후처리가 `.agents/skills` mirror에서 `metadata.json`을 바로 읽다가 실패하던 문제를 고쳤습니다.
...
- Updated dependencies [ca3f71c]
  - @create-rn-miniapp/scaffold-templates@0.1.7
